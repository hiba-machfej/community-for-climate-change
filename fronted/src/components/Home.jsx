import { useState, useEffect } from "react";
import { useHistory } from "react-router-dom";
const Home = (userData, handleFetch) => {
  const [id, setId] = useState({});
  const [event, setEvent] = useState({
    title: "",
    imgUrl: "",
    description: "",
  });
  const [events, setEvents] = useState(null);
  const [fetching, setFetching] = useState(0);
  console.log(fetching);
  const handlChange = (e) => {
    setEvent({ ...event, [e.target.name]: e.target.value });
  };

  useEffect(() => {
    const fetchEvents = async () => {
      const data = await fetch("http://localhost:3001/get-event", {
        headers: {
          Authorization: `somesupersecretsecret ${userData.token}`,
        },
      });
      const events = await data.json();
      setEvents(events);
    };
    fetchEvents();
  }, [fetching]);

  const handlDelete = async (e) => {
    const settings = {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${userData.token}`,
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: JSON.stringify(id),
    };
    try {
      const fetchResponse = await fetch(
        `http://localhost:3001/delete-event`,
        settings
      );
      const data = await fetchResponse.json();
      return data;
    } catch (e) {
      return e;
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const settings = {
      method: "POST",
      headers: {
        Authorization: `Bearer ${userData.token}`,
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: JSON.stringify(event),
    };
    try {
      const fetchResponse = await fetch(
        `http://localhost:3001/add-event`,
        settings
      );
      const data = await fetchResponse.json();
      console.log(data);
      return data;
    } catch (e) {
      return e;
    }
  };
  return (
    <div>
      {userData.name ? (
        <div>
          <h1 className="text-green-800 text-2xl ">Welcome {userData.name}</h1>
          {events ? (
            <div>
              <h3 className="text-green-700 text-xl my-3">Your Events</h3>
              <ul className="flex flex-col">
                {events.map((event, i) => (
                  <div className="flex justify-between my-2 items-center">
                    <li key={`${i}`} className="text-green-600 text-md ml-5">
                      {event.title}
                    </li>

                    <button
                      className="font-bold py-2 px-4 mr-3 rounded-md bg-green-600"
                      onClick={() => {
                        setId({ id: event.id });
                        handlDelete();
                      }}>
                      <svg
                        version="1.1"
                        id="Capa_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        fill="white"
                        x="0px"
                        y="0px"
                        width="1.2rem"
                        viewBox="0 0 482.428 482.429"
                        style={{ enableBackground: "new 0 0 482.428 482.429" }}
                        xmlSpace="preserve">
                        <g>
                          <g>
                            <path
                              d="M381.163,57.799h-75.094C302.323,25.316,274.686,0,241.214,0c-33.471,0-61.104,25.315-64.85,57.799h-75.098
			c-30.39,0-55.111,24.728-55.111,55.117v2.828c0,23.223,14.46,43.1,34.83,51.199v260.369c0,30.39,24.724,55.117,55.112,55.117
			h210.236c30.389,0,55.111-24.729,55.111-55.117V166.944c20.369-8.1,34.83-27.977,34.83-51.199v-2.828
			C436.274,82.527,411.551,57.799,381.163,57.799z M241.214,26.139c19.037,0,34.927,13.645,38.443,31.66h-76.879
			C206.293,39.783,222.184,26.139,241.214,26.139z M375.305,427.312c0,15.978-13,28.979-28.973,28.979H136.096
			c-15.973,0-28.973-13.002-28.973-28.979V170.861h268.182V427.312z M410.135,115.744c0,15.978-13,28.979-28.973,28.979H101.266
			c-15.973,0-28.973-13.001-28.973-28.979v-2.828c0-15.978,13-28.979,28.973-28.979h279.897c15.973,0,28.973,13.001,28.973,28.979
			V115.744z"
                            />
                            <path
                              d="M171.144,422.863c7.218,0,13.069-5.853,13.069-13.068V262.641c0-7.216-5.852-13.07-13.069-13.07
			c-7.217,0-13.069,5.854-13.069,13.07v147.154C158.074,417.012,163.926,422.863,171.144,422.863z"
                            />
                            <path
                              d="M241.214,422.863c7.218,0,13.07-5.853,13.07-13.068V262.641c0-7.216-5.854-13.07-13.07-13.07
			c-7.217,0-13.069,5.854-13.069,13.07v147.154C228.145,417.012,233.996,422.863,241.214,422.863z"
                            />
                            <path
                              d="M311.284,422.863c7.217,0,13.068-5.853,13.068-13.068V262.641c0-7.216-5.852-13.07-13.068-13.07
			c-7.219,0-13.07,5.854-13.07,13.07v147.154C298.213,417.012,304.067,422.863,311.284,422.863z"
                            />
                          </g>
                        </g>
                      </svg>
                    </button>
                  </div>
                ))}
              </ul>
            </div>
          ) : (
            <h3 className="text-green-700 text-md">You have no events yet</h3>
          )}
          <h3 className="text-green-700 text-xl my-3">Add new Event</h3>
          <form onSubmit={handleSubmit}>
            <div className="flex flex-col justify-between items-between">
              <div className="mb-2">
                <label className="text-md w-px" htmlFor="title">
                  Title
                </label>
                <input
                  className="border-2 max-w-xs"
                  type="text"
                  name="title"
                  required
                  value={event.title}
                  onChange={handlChange}
                />
              </div>
              <div className="mb-2">
                <label className="text-md w-px" htmlFor="password">
                  Description
                </label>
                <input
                  className="border-2 min-w-xs max-w-xs"
                  type="description"
                  name="description"
                  value={event.description}
                  onChange={handlChange}
                />
              </div>
              <div>
                <label className="text-md w-px" htmlFor="date">
                  Date
                </label>
                <input
                  className="border-2 min-w-xs max-w-xs"
                  type="date"
                  name="date"
                  value={event.date}
                  onChange={handlChange}
                />
              </div>
            </div>
            <button
              className="text-white font-bold py-2 px-4 rounded-md bg-green-600 my-5"
              type="submit">
              Submit
            </button>
          </form>
        </div>
      ) : (
        <h1>Please sign in</h1>
      )}
    </div>
  );
};
export default Home;
